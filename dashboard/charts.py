...
def aggregate_data(df, column, aggregation):
    ...
    if total_days <= 31:
        ...
        data["group"] = data["dt_iso"].dt.to_period("M").astype(str)
        group_column = "group"
    ...
    if aggregation == "mean":
        result = data.groupby(group_column, as_index=False)[column].mean()

    else:
        result = data.groupby(group_column, as_index=False)[column].sum()
...
# MENAMPILKAN GRAFIK
def show_trend_chart(df: pd.DataFrame, selected_variable: str):

    config = CHART_CONFIG[selected_variable]

    chart_data, x_column, aggregation_name = aggregate_data(
        df=df, column=config["column"], aggregation=config["aggregation"]
    )

    st.caption(f"Agregasi Data : **{aggregation_name}**")
