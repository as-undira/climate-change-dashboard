...
def weather_distribution(df: pd.DataFrame):
    ...
    data = (
        df["weather_main"].value_counts(normalize=True).mul(100).round(2).reset_index()
    )
    ...
    utama = data[data["Persentase"] >= 1].copy()

    lainnya = data[data["Persentase"] < 1]["Persentase"].sum()

    if lainnya > 0:

        utama.loc[len(utama)] = ["Lainnya", round(lainnya, 2)]

    utama["Legenda"] = (
        utama["Kondisi Cuaca"] + " (" + utama["Persentase"].astype(str) + "%)"
    )
...
def extreme_climate_events(df: pd.DataFrame):
    ...
    with col1:
        ...
        highest_temp = df.nlargest(10, "temp")[
            [
                "dt_iso",
                "temp",
                "humidity",
        ...
        highest_temp["Tanggal"] = pd.to_datetime(highest_temp["Tanggal"]).dt.strftime(
            "%d-%m-%Y"
        )

        highest_temp["Suhu (°C)"] = highest_temp["Suhu (°C)"].round(2)

        highest_temp["Kelembapan (%)"] = highest_temp["Kelembapan (%)"].round(2)
    ...
    with col2:
        ...
        highest_rain = df.nlargest(10, "rain_1h")[
            [
                "dt_iso",
                "rain_1h",
                "weather_main",
        ...
        highest_rain["Tanggal"] = pd.to_datetime(highest_rain["Tanggal"]).dt.strftime(
            "%d-%m-%Y"
        )

        highest_rain["Curah Hujan (mm)"] = highest_rain["Curah Hujan (mm)"].round(2)
